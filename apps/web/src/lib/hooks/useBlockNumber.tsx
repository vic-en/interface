...
export function BlockNumberProvider({ children }: PropsWithChildren) {
    ...
    setChainBlock((chainBlock) => {
      if (chainBlock.chainId === chainId) {
        if (!chainBlock.block || chainBlock.block < block) {
          const mainnetBlock = chainId === UniverseChainId.Wonder ? block : chainBlock.mainnetBlock
          return { chainId, block, mainnetBlock }
        }
      } else if (chainId === UniverseChainId.Wonder) {
  ...
  useEffect(() => {
    RPC_PROVIDERS[UniverseChainId.Wonder]
      .getBlockNumber()
      .then((block) => onChainBlock(UniverseChainId.Wonder, block))
